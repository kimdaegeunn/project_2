...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        // 정렬 방식에 따른 SQL 쿼리 작성
        let orderByClause = 'ORDER BY r.created_at DESC'; // 기본적으로 최신순 정렬

        if (req.query.sort === 'views_desc') {
            orderByClause = 'ORDER BY r.views DESC, r.created_at DESC'; // 조회수 내림차순, 최신순
        }
        ...
        if (req.query.searchType && req.query.searchInput) {
            ...
            if (searchType === 'title') {
                searchCondition = ` AND r.title LIKE '%${searchInput}%'`;
            } else if (searchType === 'content') {
                searchCondition = ` AND r.content LIKE '%${searchInput}%'`;
        ...
        const sql_query = `SELECT
                               id, title, author, to_char(created_at,'YYYY-MM-DD') AS created_date, r.views,
                               (SELECT COUNT(*) FROM comments c WHERE c.post_id = r.id) AS comments_count
             FROM (
                      SELECT
                          r.id, r.title, u.username AS author, r.created_at, r.views,
