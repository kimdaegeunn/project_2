...
router.get('/:id', async (req, res) => {
    ...
    try {
        ...
        // 조회수 증가 처리
        await conn.execute(
            `UPDATE campingtip SET views = views + 1 WHERE id = :id`,
            [postId]
        );
        ...
        const postResult = await conn.execute(
            `SELECT t.id, t.title, u.username AS author, t.content, TO_CHAR(t.created_at, 'YYYY-MM-DD') AS created_at, t.views 
            FROM campingtip t
